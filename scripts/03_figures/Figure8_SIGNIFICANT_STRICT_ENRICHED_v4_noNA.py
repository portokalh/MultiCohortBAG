...
DEFAULT_OUTDIR_NAME = "Figure8_SIGNIFICANT_STRICT_ENRICHED_v4_noNA"
...
def contributing_cohorts_for_endpoint(stats: pd.DataFrame, endpoint: str, min_n: int) -> List[str]:
    rows = stats[
        (stats["endpoint"] == endpoint)
        & (~stats["cohort"].eq("Pooled"))
        & (stats["status"] == "ok")
        & (stats["n"] >= min_n)
        & stats["r"].notna()
        & stats["p"].notna()
    ].copy()
    return rows["cohort"].astype(str).tolist()


def select_main_panel_rows(stats: pd.DataFrame, alpha: float, min_n: int, max_panels: int = 4) -> pd.DataFrame:
    """
    Select exact rows for panels B-E.

    Unlike endpoint-only selection, this keeps the cohort context. If an endpoint is
    significant only in HABS, the panel is plotted as HABS only. If it is significant
    only in ADNI, the panel is plotted as ADNI only. A Pooled panel is used only when
    at least two real cohorts contribute data for that endpoint.
    """
    ranked = significant_endpoint_rank(stats, alpha=alpha, min_n=min_n)
    if ranked.empty:
        return ranked
    ...
    for _, row in ranked.iterrows():
        ...
        if cohort == "Pooled":
            contributors = contributing_cohorts_for_endpoint(stats, endpoint, min_n=min_n)
            if len(contributors) < 2:
                # Do not label single-cohort data as pooled. Use the significant
                # real-cohort row when available.
                real = valid[
                    (valid["endpoint"] == endpoint)
                    & (~valid["cohort"].eq("Pooled"))
                    & (valid["p"] < alpha)
                ].sort_values(["p", "abs_r"], ascending=[True, False])
                if not real.empty:
                    candidate_rows.append(real.iloc[0].to_dict())
                    continue
    ...
    # Prefer significant rows. If there are >4 significant rows, keep unique endpoints
    # with the smallest p-values. If <4, fill with best remaining valid rows.
    cand["is_significant"] = cand["p"] < alpha
    cand["abs_r"] = cand["r"].abs()
    cand = cand.sort_values(["is_significant", "p", "abs_r", "n"], ascending=[False, True, False, False])

    selected = []
    used_endpoints = set()
    for _, row in cand.iterrows():
        e = row["endpoint"]
        if e in used_endpoints:
            continue
...
def plot_r_heatmap(ax: plt.Axes, stats: pd.DataFrame, endpoints: Sequence[str]) -> None:
    ...
    # For this no-NA version, the heatmap only displays endpoints with finite r
    # for all displayed columns. This avoids blank/white NA holes without
    # inventing values. Endpoints with partial availability are still shown in
    # the availability heatmap and forest plot.
    complete_endpoints = []
    for e in endpoints:
        vals = []
        for c in cohorts:
            row = stats[(stats["endpoint"] == e) & (stats["cohort"] == c)]
            vals.append(np.nan if row.empty else row.iloc[0]["r"])
        if all(np.isfinite(v) for v in vals):
            complete_endpoints.append(e)

    if not complete_endpoints:
        # Fallback: show endpoints with any valid r, but still no white holes by
        # dropping all-empty columns and rows.
        complete_endpoints = []
        for e in endpoints:
            if stats[(stats["endpoint"] == e) & stats["r"].notna()].shape[0] > 0:
                complete_endpoints.append(e)

    if not complete_endpoints:
        ax.text(0.5, 0.5, "No complete association heatmap", ha="center", va="center")
        ax.axis("off")
        return

    mat = pd.DataFrame(index=complete_endpoints, columns=cohorts, dtype=float)
    for e in complete_endpoints:
        for c in cohorts:
            row = stats[(stats["endpoint"] == e) & (stats["cohort"] == c)]
            mat.loc[e, c] = row.iloc[0]["r"] if not row.empty else np.nan

    # If fallback path introduced columns with any NaN, drop rows containing NaN.
    mat = mat.dropna(axis=0, how="any")
    if mat.empty:
        ax.text(0.5, 0.5, "No endpoint has complete ADNI/HABS/Pooled r values", ha="center", va="center")
        ax.axis("off")
        return
    ...
    ax.set_yticklabels([nice_label(e) for e in mat.index], fontsize=7)
    for i in range(len(mat.index)):
        for j in range(len(mat.columns)):
            val = arr[i, j]
            ax.text(j, i, f"{val:.2f}", ha="center", va="center", fontsize=7)
    ax.set_title("Longitudinal Δ association r\n(complete ADNI/HABS/Pooled endpoints only)")
    cb = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cb.set_label("r")
...
def scatter_delta_focus(ax: plt.Axes, df: pd.DataFrame, endpoint: str, stats_row: pd.Series) -> None:
    """Scatter plot for one selected significant row, preserving cohort context."""
    delta_col = f"delta__{endpoint}"
    if delta_col not in df.columns:
        ax.text(0.5, 0.5, f"No {endpoint} delta", ha="center", va="center")
        ax.axis("off")
    ...
    focus_cohort = str(stats_row.get("cohort", "Pooled"))
    plot_df = df.copy() if focus_cohort == "Pooled" else df[df["cohort"].astype(str).eq(focus_cohort)].copy()

    plotted = False
    for cohort in sorted(plot_df["cohort"].dropna().unique()):
    ...
    x_all = clean_numeric(plot_df["delta_cbag"])
    y_all = clean_numeric(plot_df[delta_col])
    ok = x_all.notna() & y_all.notna()
    if ok.sum() >= 3 and x_all[ok].nunique() >= 2:
        coef = np.polyfit(x_all[ok], y_all[ok], 1)
        xx = np.linspace(float(x_all[ok].min()), float(x_all[ok].max()), 100)
    ...
    r = stats_row.get("r", np.nan)
    p = stats_row.get("p", np.nan)
    n = int(stats_row.get("n", ok.sum()))
    label = f"{focus_cohort} n={n}, r={r:.2f}, p={p:.2g}" if np.isfinite(r) and np.isfinite(p) else f"{focus_cohort} n={n}"

    ax.axvline(0, linewidth=0.8, alpha=0.6)
    ax.axhline(0, linewidth=0.8, alpha=0.6)
    ax.set_xlabel("ΔcBAG")
    ax.set_ylabel(f"Δ{nice_label(endpoint)}")
    ax.set_title(f"ΔcBAG vs Δ{nice_label(endpoint)}\n{label}", fontsize=10)
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=7, frameon=False)


def make_figure8(delta_df: pd.DataFrame, stats: pd.DataFrame, main_endpoints: Sequence[str],
                 outdir: Path, formats: Sequence[str], dpi: int, title: str,
                 main_panel_rows: Optional[pd.DataFrame] = None) -> List[str]:
    ...
    if main_panel_rows is None or len(main_panel_rows) == 0:
        main_panel_rows = pd.DataFrame()

    panel_rows = main_panel_rows.head(4).copy()

    for i in range(1, 5):
        if i - 1 >= len(panel_rows):
            ax[i].text(0.5, 0.5, "No additional significant endpoint", ha="center", va="center")
            ax[i].axis("off")
        else:
            row = panel_rows.iloc[i - 1]
            scatter_delta_focus(ax[i], delta_df, str(row["endpoint"]), row)
    ...
    return save_all(fig, outdir / "Figure8_SIGNIFICANT_STRICT_ENRICHED_v4_noNA", formats, dpi)
...
def make_supp_figure8(delta_df: pd.DataFrame, stats: pd.DataFrame, endpoints: Sequence[str],
    ...
    fig.suptitle(title, fontsize=15, y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    return save_all(fig, outdir / "Supplementary_Figure8_SIGNIFICANT_STRICT_ENRICHED_v4_noNA", formats, dpi)
...
def main() -> None:
    ...
    main_panel_rows = select_main_panel_rows(
        stats,
        alpha=args.main_alpha,
        min_n=args.min_n,
        max_panels=4,
    )
    main_endpoints = main_panel_rows["endpoint"].astype(str).tolist() if not main_panel_rows.empty else []
    ...
    if not main_panel_rows.empty:
        main_panel_rows.to_csv(selected_panels_path, index=False)
    else:
    ...
    print("\nSelected main panel rows:")
    print(main_panel_rows.to_string(index=False) if not main_panel_rows.empty else "none")
    ...
    fig8_outputs = make_figure8(
        ...
        main_panel_rows=main_panel_rows,
    )
