...
def parse_runtime_args():
    ...
    parser.add_argument(
        "--combine-only",
        action="store_true",
        help="Skip per-cohort validation reruns and only rebuild combined outputs from existing validation_summary.csv files.",
    )
        ...
        for fs in FEATURE_SETS_TO_RUN:
            ...
            for validation_mode in VALIDATION_OUTPUT_MODES:
                ...
                if os.path.exists(cross_path):
                    try:
                        tmp = pd.read_csv(cross_path)
                        tmp["validation_mode"] = validation_mode
                        cross_frames.append(tmp)
                    except Exception as e:
                        print(f"Could not read {cross_path}: {e}")

                if os.path.exists(long_path):
                    try:
                        tmp = pd.read_csv(long_path)
                        tmp["validation_mode"] = validation_mode
        ...
        for fs in FEATURE_SETS_TO_RUN:
            ...
            for validation_mode in VALIDATION_OUTPUT_MODES:
                ...
                if os.path.exists(graph_path):
                    try:
                        tmp = pd.read_csv(graph_path)
                        tmp["validation_mode"] = validation_mode
                        graph_frames.append(tmp)
                    except Exception as e:
        ...
        for fs in FEATURE_SETS_TO_RUN:
            ...
            for validation_mode in VALIDATION_OUTPUT_MODES:
                ...
                if os.path.exists(bag_path):
                    try:
                        tmp = pd.read_csv(bag_path)
                        tmp["validation_mode"] = validation_mode
                        bag_frames.append(tmp)
                    except Exception as e:
                        print(f"Could not read {bag_path}: {e}")

                if os.path.exists(pca_path):
                    try:
                        tmp = pd.read_csv(pca_path)
                        tmp["validation_mode"] = validation_mode
                        pca_frames.append(tmp)
                    except Exception as e:
...
def collect_existing_validation_summaries() -> List[Dict]:
    """
    Combine-only helper.

    Reads existing validation_summary.csv files from:
        ablation_<feature_set>/validation_figures_oof/
        ablation_<feature_set>/validation_figures_full_cohort/

    This avoids rerunning the expensive per-cohort validation blocks.
    """
    rows = []
    for cohort in COHORTS_TO_RUN:
        for fs in FEATURE_SETS_TO_RUN:
            ...
            for validation_mode in VALIDATION_OUTPUT_MODES:
                val_outdir = os.path.join(paths["ablation_dir"], f"validation_figures_{validation_mode}")
                summary_path = os.path.join(val_outdir, "validation_summary.csv")
                if not os.path.exists(summary_path):
                    rows.append({
                        "validation_mode": validation_mode,
                        "cohort": cohort,
                        "feature_set": fs,
                        "status": "missing_validation_summary",
                        "val_outdir": val_outdir,
                ...
                try:
                    ...
                    if len(df) == 0:
                        rows.append({
                            ...
                            "cohort": cohort,
                            "feature_set": fs,
                            "status": "empty_validation_summary",
                            "val_outdir": val_outdir,
                            "summary_path": summary_path,
                        })
                    else:
                        for _, r in df.iterrows():
                            d = r.to_dict()
                            d.setdefault("validation_mode", validation_mode)
                            d.setdefault("cohort", cohort)
                            d.setdefault("feature_set", fs)
                            d["status"] = d.get("status", "ok")
                            d["summary_path"] = summary_path
                            rows.append(d)
                except Exception as e:
                    rows.append({
                        "validation_mode": validation_mode,
                        "cohort": cohort,
                        "feature_set": fs,
                        "status": "error_reading_validation_summary",
                        "error": str(e),
                        "val_outdir": val_outdir,
                        "summary_path": summary_path,
                    })
...
def main():
    ...
    if getattr(RUNTIME_ARGS, "combine_only", False):
        print("\n" + "#" * 90)
        print("COMBINE-ONLY MODE: reading existing validation_summary.csv files")
        print("#" * 90)
        validation_summaries = collect_existing_validation_summaries()
        print(f"Collected validation summary rows: {len(validation_summaries)}")
